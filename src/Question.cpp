...
std::string Question::NextLongMultiplication() {
  ...
  std::string spacing(
      static_cast<size_t>(std::to_string(m_LongMultiplication[0]).length() -
                          std::to_string(m_LongMultiplication[1]).length()),
      ' ');
...
std::string Question::NextShortDivision() {
  ...
  output += " " + std::to_string(m_ShortDivision[0]) + " " + char(246) + " " +
            std::to_string(m_ShortDivision[1]);

  return output;
}

std::string Question::LongMultiplicationAnswer() {
  m_LongMultiplicationAnswer =
      m_LongMultiplication[0] * m_LongMultiplication[1];

  std::string output =
      " The answer is " + std::to_string(m_LongMultiplicationAnswer);
...
std::string Question::ShortDivisionAnswer() {
  ...
  std::string output =
      " The answer is " + std::to_string(m_ShortDivisionAnswer[1]);
...
void Question::ShortDivisionAnswerUserAnswer() {
  ...
  if (m_ShortDivisionAnswer[0] == m_ShortDivisionUserAnswer[0] &&
...
std::string Question::LongMultiplicationSolution() {
  int TopLength =
      static_cast<int>(std::to_string(m_LongMultiplication[0]).length());
  int BottomLength =
      static_cast<int>(std::to_string(m_LongMultiplication[1]).length());
